...
class MLP_2HL(nn.Module):
    def __init__(self, dim_in, dim_hidden1, dim_hidden2, dim_output, sparse=False, bn=True):
        ...
        self.out_layer = nn.Linear(dim_hidden2, dim_output)
        self.bn = nn.BatchNorm1d(dim_hidden1)
        self.bn2 = nn.BatchNorm1d(dim_in)

    def forward(self, x, lower_f):
        if lower_f is not None:
            if len(lower_f.shape) == 1:
                # print(x.shape)
                # print(lower_f.shape)
                lower_f = lower_f.unsqueeze(0)
            x = torch.cat([x, lower_f], dim=1)
            x = self.bn2(x) if len(x) > 1 else x
        out = self.lrelu(self.in_layer(x))
        out = self.bn(out) if len(out) > 1 else out
    ...
    def get_model(cls, stage, opt):
        if stage == 0:
            ...
            dim_in = opt.feat_d + opt.hidden_d
        model = MLP_2HL(dim_in, opt.hidden_d, opt.hidden_d, opt.n_particles * opt.d_particles, opt.sparse)
        return model
