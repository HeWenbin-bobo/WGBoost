...
class DynamicNet(object):
    def __init__(self, c0, lr):
        ...
        self.c0 = c0
        self.lr = lr
        self.boost_rate = nn.Parameter(torch.tensor(lr, requires_grad=True, device="cuda" if torch.cuda.is_available() else "cpu"))
    ...
    def forward(self, x):
        if len(self.models) == 0:
            return None, self.c0.expand(x.shape[0], self.c0.shape[0], self.c0.shape[1])
        ...
        with torch.no_grad():
            for m in self.models:
                if middle_feat_cum is None:
                    ...
                    prediction += pred
        prediction = prediction.reshape(x.shape[0], self.c0.shape[0], self.c0.shape[1])
        return middle_feat_cum, self.c0.expand(x.shape[0], self.c0.shape[0], self.c0.shape[1]) + self.boost_rate * prediction
